...
```bash
# Stash current state
git stash push -m "modify-agent: pre-delete backup"
...
```bash
# Stash backup
git stash push -m "modify-agent: pre-rename backup"
...
git stash push -m "modify-agent: auto-backup"
...
### Delete
```
/supernova modify delete src/old-module.py
/supernova modify delete --dry-run src/old-module.py
/supernova modify delete --force src/old-module.py
```

### Rename
```
/supernova modify rename get_user_data get_user
/supernova modify rename --scope=src/ old_name new_name
```

### Bulk Update
```
/supernova modify bulk "API_VERSION = \"v1\"" "API_VERSION = \"v2\""
/supernova modify bulk --include="*.py" --exclude="*/test/*" search replace
```

### Refactor
```
/supernova modify refactor extract-function process_order validate_order
/supernova modify refactor extract-class UserService
```
...
**Called by:**
- User command `/supernova modify`
- Other agents needing safe modifications

**Calls:**
- `guard-agent` (security scan)
- `build-agent` (verify after changes)
